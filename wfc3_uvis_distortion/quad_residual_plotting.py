...
residual plots for each filter. This code must be ran with python 3.5 for correct rms. 
...
python quad_residual_tweakreg.py --path='/grp/hst/wfc3v/martlin/Myles_GD_files/medium_filter_test/F410M/old_files' --filter='F####' --new_or_old_version='old'
...
def quad_residual_plotting_main(path,filter,new_or_old_version):

	os.chdir(path)
	os.system('pwd')               
	list=sorted(glob.glob('*fit.match'))
	image=sorted(glob.glob('*_flc.fits'))
	
	for file,im in zip(list, image): 	
		with open(file) as f:
		...
		#------------------------------------------------
			...
			ax1.set_xlim([np.min(table[:,0])-500, np.max(table[:,0])+500])
...
if __name__ == '__main__':
	...
	quad_residual_plotting_main(args.path, args.filter, args.new_or_old_version)
